...
export const uploadFiles = async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const file of files) {
      ...
      const storagePath = `${userId}/${Date.now()}_${file.originalname.replace(/[^a-zA-Z0-9.-]/g, '_')}`;
      ...
      if (insertErr || !insertedFile) {
        ...
        processedFiles.push(insertedFile);
      }
      ...
      await supabaseAdmin.from('activity_logs').insert({
        ...
        workspace_id: workspaceId,
        action: 'file_upload',
        details: { filename: file.originalname, file_type: fileType, size_bytes: file.size }
      });
    }
